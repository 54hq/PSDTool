...
function CheckImageOutOfBounds() {
    ...
    if (errorLayerList.length > 0) {
        alert("发现 " + errorLayerList.length + " 个图层超出画布范围");
        Layer.selectLayers(errorLayerList);
    } else {
        alert("没有图层超出画布范围");
    }
...
function SelectImgPrefixLayers() {
    ...
    if (targetLayers.length > 0) {
        ...
        alert("已选中 " + targetLayers.length + " 个相关图层");
    } else {
        alert("未找到以 Img 或 9Img 开头的图层");
    }
}
//#endregion
...
function SelectEffectOrModeLayers() {
    ...
    if (targetLayers.length > 0) {
        Layer.selectLayers(targetLayers);
        alert("已选中 " + targetLayers.length + " 个带效果或特殊模式的图层");
    } else {
        alert("未找到带效果或特殊模式的图层");
    }
}
//#endregion
...
function CheckAllLayersOpen() {
    ...
    if (closedLayers.length > 0) {
        alert("发现 " + closedLayers.length + " 个未打开的图层，已全部打开");
    } else {
        alert("所有图层均已打开");
    }
}
